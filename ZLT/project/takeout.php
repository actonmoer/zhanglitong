...
{   
	...
	//禁止在自己店铺下单
		$self=$db2->get("takeout_message","userid",["id"=>$_GET['id']]);
		if($_userid==$self) {
		die("<script>alert('禁止在自己店铺下单');window.location.href='takeout.php'</script>");
	}
	...
	// }
	//控制前端页面可以访问
    if($_GET['new_takeout'] || $_userid=='0000'){
	   //修改对象位数组
	   foreach($data['cart_data'] as $k=>$v){
			if($v['is_spec'] == 1){
			   $new_spec_list =array();
			   foreach($v['spec_list'] as $v2){
				   	$new_spec_list[]=$v2;
			   }
               $data['cart_data'][$k]['spec_list']= $new_spec_list;
			}
	   }
	   $new_spec_list =array();
	   foreach($data['cart_data'] as $k=>$v){
	   ...
	   $data['cart_data'] = $new_spec_list;
	   $put = array('code'=>200,'lists'=>$lists,'delivery_time'=>$delivery_time,'data'=>$data,'takeout_shop'=>$takeout_shop,'subsidy'=>$subsidy);
...
{   
	...
	$for_sql   = "select a.id as oid,a.order_id,a.seller as seller_uid,a.restaurant_id,a.buyer_address,a.amount,a.actual_payment,a.addtime,a.status,b.thumb,b.take_out_shop_name,c.username as seller,c.sellersubsidy from $table as a left join $table1 as b on a.restaurant_id=b.id INNER JOIN {$DT_PRE}takeout_opentime AS d ON b.id=d.restaurant_id  LEFT JOIN {$DT_PRE}member AS c ON a.seller=c.userid where $condition order by a.addtime desc ";
	...
	if ($_POST['takeout_zlt']) {
        ...
        jpush_message($jpushtype,$jpushurl,$jpushapiuserid,$jpushalert,$jpushsound);
		...
		// 打印单			传入对应的 order_id 即可
		$code=$db2->select("user_printer","machinecode",["userid"=>$data['seller_uid']]);//'4004537138';//此处获取商家打印机设备号		
		if(!empty($code)){
			// -----------------------------------          订单详情               -------------------------------------------------------------
			// $condition.=" a.order_id = '" . $order_id . "' AND a.seller='".$_userid."'";
			$order_print = $db2->get("takeout_order","*",["order_id"=>$order_id]);
			// -----------------------------------          菜式详情               -------------------------------------------------------------
			$conditio=" order_id = '" . $order_id . "'";		
			$for_sql   = "select dishes_name,goods_number,sell_price from $table2  where $conditio order by id desc ";
			$dish_print = [];
			$result = $db->query($for_sql);		
			while ($r = $db->fetch_array($result)) {
				$dish_print[] = $r;
			...
			$db->free_result($result);
			// 调起易联云api
			require_once (__DIR__.'/CloudPrintApi/printDemo.php');
			$tokens='a14980beaed14983b22bfcd76ef5896f';
			// var_dump($code);die;
			for($i=0;$i<count($code);$i++){
				$array_datafrom=CloudPrintTakeout($code[$i],$order_print,$dish_print,$tokens);
				$yly_id = $array_datafrom['body']['id'];
				$zlt_id = $order_id;
				$sql1 = "(zlt_id,yly_id)";
		        $sql2 = "('$zlt_id','$yly_id')";
	            $db->query("INSERT INTO {$DT_PRE}takeout_print $sql1 VALUES $sql2");
			}
			//获取打印信息存入数据库
			//var_dump($array_datafrom);die;
	
		}
		if($_POST['check_html']){
			 die("<script>alert('支付成功');window.location.href='/mobile/Pay/PaySuccess.html?status=takeout'</script>");
		}else{
