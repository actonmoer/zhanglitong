...
function Categorytw(wbj){
	ajaxData.catid = $(wbj).attr("data-val");
	getDefaultAjax();
	datak={"for_ajax":"takeout_restaurant_list",'catid':$(wbj).attr("data-val"),"page":ajaxData.page};
	$('.screening .Brand').html($(wbj).text());
	$('.Category-eject').removeClass('grade-w-roll');
...
function Sorts(s){
	console.log(s);
	ajaxData.order = $(s).attr("data-val");
	getDefaultAjax();
	// datak={"for_ajax":"takeout_restaurant_list",'order':$(sbj).attr("data-val"),"page":ajaxData.page};
	$('#order').text($(s).text());
	$('.Sort-eject').removeClass('grade-w-roll');
	default_ajax_restaurant_list(ajaxData.order);
}
